...
Some commands can have multiple classes, indicated with `:className`; eg `[autoType:greenBg, "this text is green"]` or, for multiple seperated by seemicolon: `[autoType:greenBg;maxWdith;paddingY2;centerText, "this text is green, centered and padded"]`
...
| ----------------- | ------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| autoType          | `[autoType, "text", 500]`                                                      | prints `text` at once, after a delay of `500`ms                                                                                                                                                                                                                                                                                                            |
|                   | `[autoType:green, "text"]`                                                     | prints `text` at once but green without a delay                                                                                                                                                                                                                                                                                                            |
|                   | `[autoType, "javascript:printThisText('aaaaa')"]`                              | prints return value of `printThisText`                                                                                                                                                                                                                                                                                                                     |
| bashProfile       | `[bashProfile]`                                                                | prints a the bashProfile, defined in the first line in this document                                                                                                                                                                                                                                                                                       |
| br                | `[br]`                                                                         | prints an additional line break                                                                                                                                                                                                                                                                                                                            |
| clear             | `[clear]`                                                                      | clears the screen content. Script commences from here. Use `[clear] [goto:nobr, 0]` to clear the screen without losing memory, start from scratch and get the white space correct. To reset the whole scene, check out `[reset]`                                                                                                                           |
| credentials       | `[credentials]`                                                                | prints password verification. If the password contains `s`, it `s`ucceeds and goes on; if it contains `f`, it `f`ails and commands on the same line with credentials are then ignored.                                                                                                                                                                     |
...
| forceType         | `[forceType, "text"]`                                                          | user input is forced to type `text`, next command starts when enter is pressed. If the `text` starts with `sudo`, a third parameter is allowed for how to deal with the credentials, see below params for `[credentials]`.                                                                                                                                 |
| freeText          | `[freeText]`                                                                   | With freeText, you can type whatever. fakeBash tries to interpret the typed string. If the command start with the following, *default scripts* get executed: `ls`, `ls -l`, `ls -l -a`, `clear`, `top`, `cd ~`, `cd folderName`, `cd ..`. If the command starts with `sudo`, eg. `sudo ls`, password credentials are asked first, then `ls` gets executed. |
| garbageCollection | `[garbageCollection, 50]`                                                      | removes first 50 lines from `body` to preserve data                                                                                                                                                                                                                                                                                                        |
| goto              | `[goto, 4]`                                                                    | jump to command index `4`; `0` for begin of script                                                                                                                                                                                                                                                                                                         |
| javascript        | `[javascript, "parent.show('elementOnDesktop')"]`                              | does anything                                                                                                                                                                                                                                                                                                                                              |
| loader            | `[loader, "Loading: \[", "▓", "░", "\]", 30, 2000, 0, 100, "Progress: ", "%"]` | Shows a loading animation with the char `▓` max repeated `30` times; `2000`ms total duration; goes from `0` to `100`;  and looks like this: ``` Loading: [▓▓▓▓▓░░░░░░░░░░░░░░░░░░░░░░░░░░] Progress: 17% ```                                                                                                                                               |
| say               | `[say, "This is spoken out loud", "en", "voice"]`                              | Says the text in a bad, browser-dependend computer voice. Specify `lang`, defaults to `en`. Dont specify `voice` as this is different on all OS and Browsers. Also prints the text to the screen like `autoType`.                                                                                                                                          |
| sleep             | `[sleep, 1000]`                                                                | waits 1000ms                                                                                                                                                                                                                                                                                                                                               |
| reset             | `[reset]`                                                                      | hard reloads everything from scratch; clears the screen. Checkout `[clear]` for only clearing the screen.                                                                                                                                                                                                                                                  |
| resetProfile      | `[resetProfile]`                                                               | reset the path in the bash profile to its initial state.                                                                                                                                                                                                                                                                                                   |
| wait              | `[wait]`                                                                       | waits for the user to press any key                                                                                                                                                                                                                                                                                                                        |
| waitForEnter      | `[waitForEnter]`                                                               | waits for the user to press enter                                                                                                                                                                                                                                                                                                                          |
