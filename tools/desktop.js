...
async function openPrinter(printContent) {
    // let diagnostics = "";
    printContent = encodeURI(printContent);
    let id = await addWindow('Print Document "WiFi Network Diagnostic Report"', 'print', `print/index.html?printText=${printContent}&pages=2`, 22,13, 800,555, false, 1112);
    // This can be opened from the lockscreen. needs to move there to display.
    if(!gebi('lockScreenColorOverlay').classList.contains('hide')) {
        moveWindowToNewParent(id, 'lockScreenColorOverlay');
    }
}
